...
app.get('/items', (req, res) => {
    client.connect(err => {
        const collection = client.db("red-onion").collection("items");
...
app.get('/features', (req, res) => {
    client.connect(err => {
        const collection = client.db("red-onion").collection("features");
        collection.find().toArray((err, documents) => {
            if (err) {
                console.log(err);
                res.status(500).send({ message: err })
...
app.post('/placeOrder', (req, res) => {
    ...
    client.connect(err => {
        const collection = client.db("red-onion").collection("orders");
